...
This is the Nextflow-adapted port of the standalone ``bin/vuegen_report.py``
script. Instead of reading ``config.yaml`` and copying files out of a published
``outdir``, it takes explicit input paths (the files Nextflow has staged into the
task work directory) and:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--report-dir', default='vuegen_report', dest='report_dir',
        help='Directory to build the VueGen section tree into.',
    )
    parser.add_argument(
        '--report-type', default='html', dest='report_type',
...
def render(report_dir: Path, report_type: str) -> None:
    ...
    subprocess.run(
        [
            ...
            '--output_directory', str(report_dir / 'rendered_report'),
        ],
        check=True,
    )
