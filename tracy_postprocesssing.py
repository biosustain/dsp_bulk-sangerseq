...
for sample_name, result_df in result_df_dict.items():
    ...
    result_df['editing_result'] = result_df.apply(lambda x: 'success' if x['alt'] == np.nan else 'mutation detected', axis=1)

    #order df columns
    result_df = result_df[['sample_name', 'chr', 'pos', 'id', 'ref', 'alt', 'qual', 'filter', 'type', 'genotype', 'basepos', 'signalpos', 'editing_result']]
...
combined_result_df = combined_result_df[['sample_name', 'chr', 'pos', 'id', 'ref', 'alt', 'qual', 'filter', 'type', 'genotype', 'basepos', 'signalpos', 'editing_result']]
